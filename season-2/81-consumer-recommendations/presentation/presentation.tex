...
% Title
%------------------------------------------%
\title[\textbf{Cannabis Data Science \#81}]{}
\author{Cannlytics}
\institute[]{\Large Cannabis Data Science \#81}
\date{August \nth{31}, 2022}
...
\item What is your favorite strain?

\begin{itemize}

\vspace{0.5\baselineskip}
\item Given consumer's history, $\overline{X}$, can we predict the product(s) with the most similar chemical profile(s) $\hat{Y}$?
%
%\vspace{0.5\baselineskip}
%\item Can we use advanced statistics (matching models) under the hood to deliver product recommendations based off of similarity of chemical composition?
...
\begin{frame}{Cannabis Data Science Tools}

The data science tools we'll use include:
...
\vspace{0.75\baselineskip}
\item {\bfseries Web scraping}: Selenium and BeautifulSoup;

\vspace{0.75\baselineskip}
\item {\bfseries PDF scraping}: PDFPlumber and OCR (Tesseract);

\vspace{0.75\baselineskip}
\item {\bfseries Cryptography (SHA256)}: For creating unique IDs that encapsulate changes in results;

\vspace{0.75\baselineskip}
\item {\bfseries NLP} for standardization (coming soon);

\vspace{0.75\baselineskip}
\item {\bfseries Matching Models}: $k$-nearest neighbor model for product recommendations;

\vspace{0.75\baselineskip}
\item {\bfseries Optimization}: Gradient descent functions for finding optimal product mixes.
...
\vspace{0.75\baselineskip}
\item Moving averages (i.e. only use $m$ most recent products in $\bar{X}_m$)

\vspace{0.75\baselineskip}
\item Creating blends, mixing. E.g. After finding 2-nearest products, $y_1$ and $y_2$, find $\alpha^*$ where

$$
\min_{\alpha^*} = \bar{X} - \left( \alpha y_1 + (1-\alpha) y_2 \right).
$$
...
\item Start somewhere, then {\bfseries iterate}.
