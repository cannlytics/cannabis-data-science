...
Data files:
    ...
    - [Florida Lab Results 2023-06-07](fl-lab-results-2023-06-07)
    - [Cannabis Plant Patents](cannabis-plant-patents-2023-06-06)
...
Data sources:
    ...
    - [FL Medical Trees Reddit](https://www.reddit.com/r/FLMedicalTrees/search/?q=COA&sort=new)
...
#-----------------------------------------------------------------------
# Explore "garlic" aromas in Florida lab results.
#-----------------------------------------------------------------------

# Read Florida lab results.
datafile = 'D://data/florida/lab_results/.datasets/fl-lab-results-2023-06-07T06-20-43.xlsx'
fl_results = pd.read_excel(datafile)

# Identify garlic strains.
garlic = fl_results.loc[
    (fl_results['product_name'].str.contains('garlic', case=False)) |
    (fl_results['strain_name'].str.contains('garlic', case=False))
]
garlic['beta_pinene'] = pd.to_numeric(garlic['beta_pinene'], errors='coerce')
garlic['d_limonene'] = pd.to_numeric(garlic['d_limonene'], errors='coerce')
...
sample_scatterplot(
    ...
    y='beta_pinene',
    label='product_name',
    outfile='fl-garlic-beta-pinene-to-d-limonene.png',
)
...
# Read patent data.
datafile = 'D://data/strains/patents/cannabis-plant-patents-2023-06-06-15-24-39.xlsx'
patents = pd.read_excel(datafile, index_col=0)
...
extracted.to_excel(f'D://data/strains/patents/cannabis-plant-patents-{date}.xlsx')
...
#-----------------------------------------------------------------------
# Future work: Visualize the extracted patent data.
#-----------------------------------------------------------------------
